...
# ------------------------------------------------
def hull_of_foci(data_fem, n_focis=1, dist='euclidean'):
    method_distance = getattr(distance, dist)
    result = []
    ...
    for i in range(2):
        ...
        for row in data_fem.iterrows():
            dist = method_distance(object_row, row[1])
    ...
    if n_focis > 3:
        edge = method_distance(data_fem.iloc[result[0]], data_fem.iloc[result[1]])
        while n_focis-2 > 0:
            smaller = [np.inf, 0]
            ...
            for row in data_fem.iterrows():
                ...
                if row[0] not in result:  # ok
                    error = 0
                    for i in result:
                        dist = method_distance(data_fem.iloc[i], row[1])
...
# ------------------------------------------------
def list_distances(search_object, data_fem, dist='euclidean'):
    method_distance = getattr(distance, dist)
    result = []
    for row in data_fem.iterrows():
        dist = method_distance(data_fem.iloc[search_object], row[1])
...
# ------------------------------------------------
def omni_candidates_refinement(candidates, data_fem, radius, search_object, dist='euclidean'):
    method_distance = getattr(distance, dist)
    result = []
    for candidate in candidates:
        dst = method_distance(data_fem.iloc[search_object], data_fem.iloc[candidate])
        if dst <= radius:
...
# ------------------------------------------------
def normalization_radius(radius, data_fem1, data_fem2, focis_fem1, focis_fem2, dist='euclidean'):  # For two data spaces
    method_distance = getattr(distance, dist)
    max_distance_fem1 = method_distance(data_fem1.iloc[focis_fem1[0]], data_fem1.iloc[focis_fem1[1]])
    max_distance_fem2 = method_distance(data_fem2.iloc[focis_fem2[0]], data_fem2.iloc[focis_fem2[1]])
